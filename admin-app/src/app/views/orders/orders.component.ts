...
export class OrdersComponent implements OnInit {
  ...
  assignWorker(orderId: string, workerId: string) {
    this.orderService.affectOrder(orderId, workerId).subscribe(
      (response) => {
        if(response.success) {
          this._snackBar.open('Phân công thành công', 'Đóng', {
            duration: 3000,
          ...
          this._snackBar.open(response.message || 'Có lỗi xảy ra khi phân công', 'Đóng', {
            duration: 3000,
            horizontalPosition: 'right',
            verticalPosition: 'top',
          });
      ...
      (error) => {
        this._snackBar.open('Có lỗi xảy ra khi phân công', 'Đóng', {
          duration: 3000,
          horizontalPosition: 'right',
          verticalPosition: 'top',
        });
